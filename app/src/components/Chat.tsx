import React, {
	useState, useRef, useEffect, useCallback, useMemo, FormEvent, UIEvent,
} from "react";
...
const Chat = (): JSX.Element => {
	...
	const {
		socket, rooms, changeRoom, currentConnectedRoomId, fetchRooms,
	} = useChat(messagesRef);
	...
	const currentMessages = useMemo(() => {
		if (rooms.length) {
			const currentRoom = rooms.find((room) => room._id === currentConnectedRoomId);
			...
			<div className="container">
				...
				<div className="rooms">
					...
					<ul>
						...
						<li className={!state.isCreateForm ? "plus" : "plus minus"}>
							<button type="button" onClick={() => setState({ ...state, isCreateForm: !state.isCreateForm })}></button>
						</li>
					</ul>
				</div>
				{
